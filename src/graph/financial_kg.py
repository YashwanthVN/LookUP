...
class DynamicFinancialKG:
    ...
    def __init__(self, quarters: int = 4, api_key: Optional[str] = None):
        ...
        # --- FIX: Environment-Agnostic Pathing ---
        ...
        self.gnn_model = FinancialReasoningGNN(in_channels=3, edge_channels=1, hidden=64, out=32)
        print("🧠 Reasoning GNN initialized.")

    def build_for_tickers(self, tickers: List[str]):
        self.graph.clear()
        # --- FIX: Graceful fail for Commodities (Gold/Silver) ---
        try:
        ...
        for ticker in tickers:
            ...
            # Sector fallback for commodities
            sector = prof.get("sector", "Commodity/Other")
            self.graph.add_node(f"company_{ticker}", type="company", sector=sector, feat=[1.0, 0.0, 0.0])
    ...
    def _add_metric_edges(self, ticker, metric, date_prefix, val):
        comp_node, met_node, time_node = f"company_{ticker}", f"metric_{metric}", f"time_{date_prefix}"
        clean_val = float(val) if val and np.isfinite(val) else 0.0
        
        # --- FIX: Restore Temporal Edges ---
        self.graph.add_edge(comp_node, met_node, relation="REPORTED", value=clean_val, is_metric=True)
    ...
    def _add_competitor_edges(self, tickers):
        for ticker in tickers:
            ...
            peers = [n for n, d in self.graph.nodes(data=True) if d.get('sector') == u_sector and n != u_node]
            
            for peer in peers:
                # --- FIX: Uses FMPClient's new historical price method ---
                corr = self._get_correlation_weight(ticker, peer.replace("company_", ""))
                self.graph.add_edge(peer, u_node, relation="COMPETITOR_INFLUENCE", value=float(corr))

    def _get_correlation_weight(self, t1, t2):
        try:
            p1, p2 = self.client.get_historical_prices(t1), self.client.get_historical_prices(t2)
            if len(p1) < 5 or len(p2) < 5: return 0.5
            return max(0.1, np.corrcoef(p1[-20:], p2[-20:])[0, 1])
        except: return 0.5

    def _normalize_metrics(self):
        for m_type in self.METRIC_TYPES:
            edges = [d for u, v, d in self.graph.edges(data=True) if f"metric_{m_type}" in v and d.get('is_metric')]
            if not edges: continue
            vals = [e['value'] for e in edges]
            mean, std = np.mean(vals), np.std(vals)
            for e in edges: e['value'] = (e['value'] - mean) / (std if std > 0 else 1.0)

    def add_news_event(self, ticker, sentiment, magnitude, headline=None):
        news_id = f"news_{ticker}_{hash(headline) % 10000}"
        self.graph.add_node(news_id, type="news", feat=[0.0, 0.0, 1.0], 
                            sentiment=float(sentiment), label=str(headline))
    ...
    def inject_real_time_news(self, ticker: str):
        ...
        items = fetcher.fetch(ticker, limit=12)   # fetch up to 12 items
        if not items:
            print(f"⚠️ No news items found for {ticker}")
            return

        # Extract headlines (skip items without headline)
        ...
        inputs = self.tokenizer(headlines, return_tensors="pt", truncation=True, padding=True, max_length=512)
        ...
        for idx, item in enumerate(items):
            ...
            # Boosting based on hints and critical keywords
    ...
    def get_node_index(self, label: str):
        ...
        try:
            ...
            # 2. Handle Tickers (Stock or Commodity)
            # Check for direct match first (e.g., XAUUSD or company_NVDA)
            if label in node_list:
                return node_list.index(label)
            
            # 3. Fallback: try with the company_ prefix
